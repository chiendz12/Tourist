...
(async () => {
  ...
  // Full ladder: submit -> leader -> lecturer publishes (no admin step)
  ...
  const finalApprove = await call('PATCH', `/approval/${approvalId}/review`, { token: gvLan.accessToken, body: { action: 'APPROVE' } });
  ok('lecturer approval publishes it', finalApprove.data.status === 'PUBLISHED', `got ${finalApprove.data.status}`);
