...
describe('ApprovalService transitions', () => {
  ...
  it('moves approval through leader, then lecturer publishes', () => {
    ...
    expect(transition(ApprovalLevel.LECTURER, ApprovalAction.APPROVE)).toEqual({ status: ApprovalStatus.PUBLISHED, level: ApprovalLevel.LECTURER });
    // Legacy ADMIN rows still finalize to PUBLISHED.
