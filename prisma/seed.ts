...
function historyFor(flow: Flow) {
  ...
  switch (flow.shape) {
    ...
    default:
      return {
        status: ApprovalStatus.PUBLISHED,
        currentLevel: ApprovalLevel.ADMIN,
        history: [
          submit,
          { actorId: leader, level: ApprovalLevel.LEADER, action: ApprovalAction.APPROVE, comment: 'Duyệt cấp nhóm.' },
          { actorId: lecturer, level: ApprovalLevel.LECTURER, action: ApprovalAction.APPROVE, comment: 'Duyệt cấp giảng viên.' },
          { actorId: USERS.admin.id, level: ApprovalLevel.ADMIN, action: ApprovalAction.APPROVE, comment: 'Publish.' },
        ],
      };
  }
}
